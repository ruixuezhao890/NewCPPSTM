...
class SoftWire : public inputStream, public outputStream {
  public:
    enum result_t {
      ack = 0,
      nack = 1,
    ...
    SoftWire(Pin_enum sda, Pin_enum scl);
  ...
  private:
    ...
    void delayMicroseconds(uint8_t us) const;

    static void pinMode(uint8_t scl, GpioMode mode);

    static uint8_t digitalRead(uint8_t scl);
};
...
{
  _inputMode =(enable ? (uint8_t )(GPIO_MODE_INPUT|GPIO_PULLUP):(uint8_t ) GPIO_MODE_INPUT);
}
...
bool SoftWire::sclHighAndStretch(AsyncDelay& timeout) const
{
  ...
  while (_readScl(this) == GPIO_PIN_RESET)
