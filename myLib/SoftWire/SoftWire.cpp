...
#endif
  {
    gpio_write((Pin_enum )sda, 0);
    pinMode(sda, GpioMode::output_pp);
  }
}
...
{
  pinMode(p->getSda(), (GpioMode)p->getInputMode());
}
...
#endif
  {
    gpio_write((Pin_enum)scl, 0);
    pinMode(scl, GpioMode::output_pp);
  }
}
...
{
  pinMode(p->getScl(), (GpioMode)p->getInputMode());
}
...
SoftWire::SoftWire(Pin_enum sda, Pin_enum scl) :
  _sda(sda),
  _scl(scl),
  _inputMode(GPIO_MODE_INPUT), // Pullups disabled by default
...
SoftWire::result_t SoftWire::llWrite(uint8_t data) const
{
  ...
  result_t res = (_readSda(this) == 0 ? ack : nack);
...
void SoftWire::delayMicroseconds(uint8_t us) const {
    delay_us(us);
}

void SoftWire::pinMode(uint8_t scl, GpioMode mode) {
    gpio_init((Pin_enum)scl,(GpioMode)mode);
}

uint8_t SoftWire::digitalRead(uint8_t scl) {
    return    gpio_read((Pin_enum)scl);
